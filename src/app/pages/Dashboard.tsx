...
function deriveStatus(gh: {
  ...
  // Current deployment has live devices only in GH-01.
  if (gh.id !== "GH-01") {
    return "离线" as const;
  }

  const temp = asNumber(gh.temp);
  const humidity = asNumber(gh.humidity);
  const light = asNumber(gh.light);

  if (temp === null && humidity === null && light === null) {
    return "离线" as const;
  }

  const outOfRange =
    (temp !== null && (temp < 18 || temp > 30)) ||
    (humidity !== null && (humidity < 50 || humidity > 80)) ||
    (light !== null && (light < 50 || light > 1000));

  return outOfRange ? ("告警" as const) : ("正常" as const);
}
...
export function Dashboard() {
  ...
  const onlineCount = greenhouseData.filter((g) => deriveStatus(g) !== "离线").length;
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {greenhouseData.map((gh) => {
            const status = deriveStatus(gh);
            return (
              <div
                key={gh.id}
                className={`bg-white rounded-xl border-2 p-4 shadow-sm hover:shadow-md transition-shadow cursor-pointer ${statusBgMap[status]}`}
              >
                {/* Card Header */}
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                  ...
                  <button
                    onClick={() => toMonitor(gh.name, status)}
